...
def display(intx,inty,Evolution):
    ...
    #surf=ax.plot_surface(x,y,z,cmap='binary',edgecolor='k')
    ...
    Z=[f_ob(Evolution[i])+0.1 for i in range(1,nr_points)]
    ax.plot3D([X[0]], [Y[0]], [f_ob(Evolution[0])+0.1], 'y*', markersize=15)
    ax.plot3D(X,Y,Z,'bo')
    ax.plot3D([X[nr_points-2]], [Y[nr_points-2]], [Z[nr_points-2]], 'r*', markersize=10)
    grafic.show()

if __name__=="__main__":
    x,fx=HC([-2,2],[-2,2],3,0.01) # - max. 3 neighbours to the left, max. 3 neighbours to the right
